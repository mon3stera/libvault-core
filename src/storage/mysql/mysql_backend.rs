...
impl MysqlBackendLock {
    ...
    fn release_lock(&mut self) -> Result<(), RvError> {
        let rows_affected = diesel::sql_query("SELECT RELEASE_LOCK(?)")
            .bind::<Text, _>(&self.lock_name)
            .execute(&mut self.conn)?;

        if rows_affected == 0 {
            log::error!(
                "MysqlBackendLock failed to release lock: {}",
                self.lock_name
            );
...
impl Drop for MysqlBackendLock {
    fn drop(&mut self) {
        if let Err(err) = self.release_lock() {
            log::error!(
                "MysqlBackendLock error releasing lock '{}': {:?}",
                self.lock_name,
                err
            );
        }
...
impl Backend for MysqlBackend {
    async fn list(&self, prefix: &str) -> Result<Vec<String>, RvError> {
        ...
        let results: Result<Vec<MysqlBackendEntry>, _> = vault
            .filter(vault_key.like(format!("{}%", prefix)))
            .load::<MysqlBackendEntry>(conn);
    ...
    async fn get(&self, key: &str) -> Result<Option<BackendEntry>, RvError> {
        ...
        let result: Result<MysqlBackendEntry, _> = vault
            .filter(vault_key.eq(key))
            .first::<MysqlBackendEntry>(conn);

        match result {
            Ok(entry) => {
                return Ok(Some(BackendEntry {
                    key: entry.vault_key,
                    value: entry.vault_value,
                }));
            }
            Err(e) => {
    ...
    async fn put(&self, entry: &BackendEntry) -> Result<(), RvError> {
        ...
        let _lock = self.lock(&entry.key).await?;

        let conn = &mut self.pool.get()?;

        let new_entry = MysqlBackendEntry {
            vault_key: entry.key.clone(),
            vault_value: entry.value.clone(),
        };
