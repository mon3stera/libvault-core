...
use crate::{
    ...
    utils::{DatabaseName, db::strip_db_name},
};
...
impl SqlxBackendLock {
        ...
        let result: bool = match self.db_scheme.as_str() {
            "mysql" => {
                let count: Option<i32> =
                    sqlx::query_scalar("SELECT GET_LOCK(SHA1(?), ?) as result")
                        .bind(lock_name)
            ...
            "postgres" => {
                let ret: Option<bool> =
                    sqlx::query_scalar("SELECT pg_advisory_lock(hashtext('?'))")
                        .bind(lock_name)
                        .fetch_one(&self.pool)
                        .await?;
                ret.unwrap_or(false)
    ...
    async fn unlock(&self, lock_name: &str) -> Result<(), RvError> {
        match self.db_scheme.as_str() {
            "mysql" => {
                sqlx::query("SELECT RELEASE_LOCK(SHA1(?))")
                    .bind(lock_name)
                    .execute(&self.pool)
                    .await?;
            }
...
impl Backend for SqlxBackend {
    async fn list(&self, prefix: &str) -> Result<Vec<String>, RvError> {
        ...
        let results: Vec<SqlxBackendEntry> = sqlx::query_as(&format!(
    ...
    async fn get(&self, key: &str) -> Result<Option<BackendEntry>, RvError> {
        ...
        let result: Option<SqlxBackendEntry> = sqlx::query_as(&format!(
    ...
    async fn lock(&self, _lock_name: &str) -> Result<Box<dyn Any>, RvError> {
        Ok(Box::new(SqlxBackendLock::new(
            &self.pool,
            self.db_scheme.as_str(),
            1,
        )))
    }
}
    ...
    async fn new_backend(conf: &HashMap<String, Value>) -> Result<Self, RvError> {
        ...
        match db_name {
            DatabaseName::MySql(database_name) => {
                let ret = sqlx::query(&format!("CREATE DATABASE IF NOT EXISTS `{database_name}`"))
                    .execute(&pool)
                    .await;
                if ret.is_err() {
